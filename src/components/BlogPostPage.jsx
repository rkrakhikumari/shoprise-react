...
// Component name is PascalCase (valid)
const BlogPostPage = () => {
  const post = {
...
These cities are where tradition intertwines with modernity, where history and culture unfold in the most captivating of ways. Join us as we embark on a journey to explore some of the most enchanting exotic cities the world has to offer, where each step promises a new adventure.`,
  ...
  return (
    <>
      <div className="max-w-6xl mx-auto px-4 md:px-6 py-24 grid grid-cols-1 md:grid-cols-2 gap-8">
        <div>
            ...
            <div className="flex gap-4 pt-3">
              {[Facebook, Twitter, linkImg].map((icon, idx) => (
                <button
                  key={idx}
                  type="button"
                  className="w-10 h-10 bg-[#1F3A93] rounded-full flex items-center justify-center cursor-pointer"
                  aria-label={`Share on ${icon === Facebook ? 'Facebook' : icon === Twitter ? 'Twitter' : 'Copy Link'}`}
                >
                  <img src={icon} alt="" aria-hidden="true" className="w-6 h-6 md:w-7 md:h-7" />
                </button>
              ))}
            </div>
          </div>
      ...
      <div className="pt-10 px-4 md:px-20 pb-3 max-w-6xl mx-auto">
        <img src={Post} alt="Featured view of Ibiza" className="w-full rounded object-cover" />
        ...
        <div className="flex flex-col items-start gap-3 mb-10 pt-6">
          ...
          <div className="flex gap-4 pt-1">
            {[
              { icon: Facebook, label: "Share on Facebook", href: "https://facebook.com" },
              { icon: Twitter, label: "Share on Twitter", href: "https://twitter.com/intent" },
            ].map((item, idx) => (
              <a
                key={idx}
                href={item.href}
                ...
                className="w-10 h-10 bg-[#1F3A93] rounded-full flex items-center justify-center cursor-pointer"
              >
                <img src={item.icon} alt="" aria-hidden="true" className="w-6 h-6 md:w-7 md:h-7" />
              </a>
            ))}
          </div>
        </div>
      ...
      <div className="flex flex-col md:flex-row items-center justify-between mt-10 mb-4 px-4 md:px-20 max-w-6xl mx-auto gap-4 md:gap-0">
        ...
        <button
          type="button"
          className="bg-[#1F3A93] text-white px-4 py-1 rounded-md text-sm whitespace-nowrap cursor-pointer"
          onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}
          aria-label="View all related blog posts"
        >
          View all
        </button>
      </div>
      ...
      <div className="px-4 md:px-20 pb-8 max-w-6xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {blogPosts.map((post) => (
            <Link
              key={post.id} 
              to="/blog/exotic-cities-to-visit"
              className="bg-white rounded-md overflow-hidden shadow-sm focus:outline-none focus:ring-2 focus:ring-[#1F3A93] cursor-pointer"
              aria-label={`Read post: ${post.title}`}
            >
              <img
                src={post.image}
                alt={post.title}
                className="w-full h-48 md:h-60 object-cover rounded-t-2xl"
              />
              <div className="p-4">
                <p className="text-blue-700 hover:underline text-sm md:text-base">
                  {post.category}
                </p>
