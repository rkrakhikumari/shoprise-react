    ...
    <div className="w-full text-[10px] sm:text-sm">
          ...
          {['Clicks on listings', 'Listing saves', 'Listing shares', 'Marketplace followers'].map((label, index) => (
            <div key={label} className="flex items-center gap-3">
              <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-[#D3D3D3] flex items-center justify-center">
                <div className="w-4 h-4 sm:w-5 sm:h-5 bg-[#1F3A93]" aria-hidden="true"></div>
              </div>
      ...
      <div className="px-2 sm:px-3 md:px-6">
        ...
        <div className="overflow-x-auto w-full">
          <table
            className="min-w-[800px] w-full bg-white text-[10px] sm:text-sm text-left"
            aria-label="User orders"
          >
            <caption className="sr-only">List of recent orders</caption>
            <thead>
              <tr className="bg-gray-100">
                <th scope="col" className="px-4 py-3 font-medium whitespace-nowrap">ID</th>
                <th scope="col" className="px-4 py-3 font-medium whitespace-nowrap">Date</th>
                <th scope="col" className="px-4 py-3 font-medium whitespace-nowrap">Customer</th>
                <th scope="col" className="px-4 py-3 font-medium whitespace-nowrap">Products</th>
                <th scope="col" className="px-4 py-3 font-medium whitespace-nowrap">Address</th>
                <th scope="col" className="px-4 py-3 font-medium whitespace-nowrap">Fulfillment</th>
                <th scope="col" className="px-4 py-3 font-medium whitespace-nowrap">Total</th>
                <th scope="col" className="px-4 py-3 font-medium whitespace-nowrap">Status</th>
              </tr>
            </thead>
            <tbody>
              {orders.map((order) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span
                      className={`text-[9px] sm:text-xs font-semibold px-3 py-1 rounded-full ${getBadgeClass(
                        'fulfillment',
                        order.fulfillment
                      )}`}
                    >
