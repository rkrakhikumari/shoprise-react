  ...
  return (
    <div>
      ...
      <div className="relative rounded-md overflow-hidden w-full h-full">
        <img src={settingImg} alt="Settings background" className="w-full h-screen object-cover" />
        ...
        <div className="absolute bottom-4 sm:bottom-10 left-4 sm:left-auto right-4 sm:right-auto sm:mx-auto bg-transparent rounded-lg p-4 sm:p-6 w-auto sm:w-80 max-w-sm mx-auto text-[#333333] border border-white">
          ...
          <div className="mt-4 sm:mt-6 flex gap-2 sm:gap-3">
            <button
              className="px-3 py-2 sm:px-4 sm:py-2 rounded border border-blue-800 text-blue-800 text-xs sm:text-sm flex-1 sm:flex-none cursor-pointer"
              onClick={() => setShowProfileModal(true)}
            >
              View profile
            </button>
            <button
              className="px-3 py-2 sm:px-4 sm:py-2 rounded bg-blue-800 text-white text-xs sm:text-sm flex-1 sm:flex-none cursor-pointer"
      ...
      {showProfileModal && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center backdrop-blur-md bg-black/30 p-4"
          role="dialog"
          aria-modal="true"
          ...
          <div className="bg-white rounded-lg p-3 sm:p-4 w-full max-w-[350px] sm:max-w-[500px] shadow-lg relative max-h-[90vh] overflow-y-auto">
            ...
            <div className="mt-4 sm:mt-5">
              ...
              <div className="flex items-center gap-3 sm:gap-4 text-xl mt-3 justify-center sm:justify-start">
                {[
                  { src: whatsappImg, label: "WhatsApp" },
                  { src: facebookImg, label: "Facebook" },
                  { src: twitterImg, label: "Twitter" },
                  { src: msngrImg, label: "Messenger" },
                  { src: mailImg, label: "Email" },
                ].map(({ src, label }, i) => (
                  <button key={`icon-${i}`} className="focus:outline-none" aria-label={label}>
                    <img
                      src={src}
                      alt={`${label} icon`}
                      className="w-6 h-6 sm:w-auto sm:h-auto cursor-pointer"
                    />
                  </button>
                ))}
              </div>
            ...
            <button
              ...
              className="absolute top-2 right-3 text-gray-500 hover:text-gray-800 text-xl sm:text-2xl cursor-pointer"
